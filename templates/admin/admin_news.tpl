...
<div class="field">
	...
	<label for="attachment" class="label">Category<span class="hintanchor" title="Allows you to categorise your news items."><img src="{$tempdir}admin/images/help.png" alt="[?]"/></span></label>
	<div class="inputboxwrapper">
		<select name="category_id" id="attachment" class="inputbox">
			<option value="0" {if $shownews.category_id == 0}selected="selected"{/if}>None</option>
			{section name=thing loop=$numcategories}
				<option value="{$categories[thing].id}" {if $shownews.category_id == $categories[thing].id}selected="selected"{/if}>{$categories[thing].name}</option>
			{/section}
		</select>
	</div><br />
</div>

<textarea id="editor" name="editor" style="width:100%; height:500px" class="inputbox">{$shownews.news}</textarea>
