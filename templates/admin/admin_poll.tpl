...
<div class="inputboxwrapper"><input type="text" name="date_stop" id="date_stop"  class="inputbox dateformat-Y-ds-m-ds-d range-low-today" {if $action=="edit" && $item.date_stop != 0}value="{$item.date_stop|date_format:"%Y-%m-%d"}"{/if} onblur="checkElement('date_stop', 'date', false, 0, 0, '');"/><br /><span class="fieldError" id="date_stopError">Must a valid date in the format: YYYY-MM-DD</span></div><br />
